...
- The populated demo report route `/studies/demo-study/report` now downloads real export artifacts when the browser session is started with a configured download path: **Export JSON** yields a report object containing `headlineMetrics`, `limitations`, and inline issue clusters, and **Export HTML** yields a self-contained HTML file.
...
- On the rerun, a fresh self-service account could import and publish a pack, create a study, and reach `persona review`, but visible variant generation remained stuck on **Generating variants...** and the live study report tab stayed **Report not ready**, so browser-only hardening validation still cannot reach a completed live analyzed report.
- The launch confirmation flow currently exposes only **Cancel** and **Confirm Launch** actions; no raw `validateStudyLaunch { pass, reasons[] }` payload or explicit production-ack checkbox is visible from the browser surface.
