...
function createRequest(runId: string): ExecuteRunRequest {
  return {
    runId,
    studyId: `${runId}_study`,
    personaVariant: {
      id: "pv_001",
...
describe("createExecuteRunHandler OPENAI_BASE_URL wiring", () => {
  it("passes env.OPENAI_BASE_URL through the default AI action selector", async () => {
    ...
    const response = await handler(createRequest("run_openai_base_url_action"), {
  ...
  it("passes env.OPENAI_BASE_URL through the default self-report generator", async () => {
    mockedGenerateWithModel.mockReset();
    mockedGenerateWithModel.mockResolvedValue(mockTextResult(JSON.stringify({
      perceivedSuccess: true,
      hardestPart: "Finding checkout",
      confusion: "None",
      confidence: 0.9,
    ...
    const response = await handler(createRequest("run_openai_base_url_self_report"), {
      BROWSER_LEASE: namespace,
      OPENAI_API_KEY: "worker-openai-key",
      OPENAI_BASE_URL: PROXY_URL,
    });
